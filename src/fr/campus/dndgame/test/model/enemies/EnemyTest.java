...
public class EnemyTest {
    ...
    private static void testConstructorAndStats() {
        try {
            Enemy g = new Goblin();
            if (g.getHealth() <= 0)
                throw new AssertionError("Les PV doivent être > 0");
            if (g.getAttack() <= 0)
                throw new AssertionError("L'attaque doit être > 0");
            if (g.getName() == null || g.getName().isEmpty())
                throw new AssertionError("Le nom ne doit pas être vide");
            if (g.getMaxHealth() != g.getHealth())
                throw new AssertionError("PV actuels doivent être égaux aux PV max au départ");
            if (g.getDefense() != 0)
                throw new AssertionError("La défense initiale doit être 0");
    ...
    private static void testGettersSetters() {
        try {
            ...
            e.setAttack(99);
            if (e.getAttack() != 99)
                throw new AssertionError("Setter/Getter Attack échoué");

            // Test Health
            e.setHealth(1);
            if (e.getHealth() != 1)
                throw new AssertionError("Setter/Getter Health échoué");

            // Test MaxHealth
            e.setMaxHealth(50);
            if (e.getMaxHealth() != 50)
                throw new AssertionError("Setter/Getter MaxHealth échoué");

            // Test Defense
            e.setDefense(10);
            if (e.getDefense() != 10)
                throw new AssertionError("Setter/Getter Defense échoué");
    ...
    private static void testToString() {
        try {
            ...
            if (s == null)
                throw new AssertionError("toString ne doit pas être null");
            if (!s.contains(e.getName()))
                throw new AssertionError("toString doit contenir le nom");
            if (!s.contains("HP"))
                throw new AssertionError("toString doit contenir 'HP'");
            if (!s.contains("attaque"))
                throw new AssertionError("toString doit contenir 'attaque'");
