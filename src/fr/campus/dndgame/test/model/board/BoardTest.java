...
public class BoardTest {
    ...
    private static void testConstructor() {
        try {
            ...
            if (b1.getSize() != 50)
                throw new AssertionError("Taille incorrecte");
            if (!b1.getCells().isEmpty())
                throw new AssertionError("Les cases doivent être vides au début");

            Board b2 = new Board(123, 20, "Donjon");
            if (b2.getId() != 123)
                throw new AssertionError("ID incorrect");
            if (!"Donjon".equals(b2.getName()))
    ...
    private static void testSetName() {
        try {
            ...
            if (!"TestBoard".equals(b1.getName()))
                throw new AssertionError("Nom incorrect");
    ...
    private static void testInitBoardLogic() {
        try {
            ...
            b.initBoard();
            if (b.getCells().size() != 50)
                throw new AssertionError("La taille ne correspond pas");
            int enemyCount = 0;
            int boxCount = 0;
            ...
            if (enemyCount != 24)
                throw new AssertionError("Compte ennemi incorrect: " + enemyCount);
            if (boxCount != 24)
                throw new AssertionError("Compte box incorrect: " + boxCount);
    ...
    private static void testGetCellBounds() {
        try {
            ...
            b.initBoard();
            // Cas normal
            if (b.getCell(5).getNumber() != 5)
                throw new AssertionError("Case 5 incorrecte");
            // Cas limite bas (doit renvoyer case 1)
            if (b.getCell(0).getNumber() != 1)
                throw new AssertionError("Gestion limite basse échouée");
            // Cas limite haut (doit renvoyer case 10)
            if (b.getCell(15).getNumber() != 10)
                throw new AssertionError("Gestion limite haute échouée");
    ...
    private static void testIsLastCell() {
        try {
            Board b = new Board(10);
            if (b.isLastCell(9))
                throw new AssertionError("9 n'est pas la fin");
            if (!b.isLastCell(10))
                throw new AssertionError("10 doit être la fin");
            if (!b.isLastCell(12))
                throw new AssertionError("12 doit être considéré comme fin");
